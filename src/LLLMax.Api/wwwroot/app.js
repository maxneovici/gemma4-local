...
async function loadAgents() {
  ...
  $('agents').innerHTML = agents.map(agent => `
    <button type="button" title="${escapeHtml(agent.description)}">${escapeHtml(agent.name)}</button>
  `).join('');
}
...
async function newSession() {
  const session = await api('/sessions', { method: 'POST', body: JSON.stringify({ title: 'LLLMax session' }) });
...
async function runDocumentAction(path) {
  if (!state.uploadedDocumentId) return;
...
renderMetrics(null);
renderSteps([]);
await Promise.all([loadModels().catch(() => {}), loadAgents(), loadSessions()]);
if (state.sessions[0]) {
